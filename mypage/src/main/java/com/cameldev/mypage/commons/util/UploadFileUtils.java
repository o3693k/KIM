...
public class UploadFileUtils {
	...
	public static String uploadFile(String uploadPath, String originalName, byte[] fileData) throws Exception {
		...
		// 파일 객제 생성 = 기본 저장경로 + 날짜 경로 + UUID_파일명
	...
	private static String calcPath(String uploadPath) {
		...
		// 년
		String yearPath = File.separator + calndar.get(Calendar.YEAR);
		// 년+월
		String monthPath = yearPath + File.separator + new DecimalFormat("00").format(calndar.get(Calendar.MONDAY) + 1);
		// 년+월+일
		String datePath = monthPath + File.separator + new DecimalFormat("00").format(calndar.get(Calendar.DATE));
		// 파일저장 기본경로+날짜 경로생성
	...
	private static void makeDir(String uploadPath, String... paths) {
		...
		for (String path : paths) {
			// 기본경로 + 날짜 경로에 해당하는 파일 객제 생성
			File dirPath = new File(uploadPath + path);
			// 파일 객제에 해당하는 경로가 존재 X
			if (!dirPath.exists()) {
				// 경로 생성
				dirPath.mkdir();
			}
