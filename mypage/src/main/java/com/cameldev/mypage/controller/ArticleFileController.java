...
public class ArticleFileController {
	...
	// 단일 파일 데이터 삭제 2 : 게시글 수정 화면
	@Transactional
	@ResponseBody
	@RequestMapping(value = "/delete/{article_no}", method = RequestMethod.POST)
	public ResponseEntity<String> boardMofifyRemoveFile(@PathVariable("article_no") Integer article_no, String fileName, HttpServletRequest request)
			throws Exception {
		// DB에서 파일명 제거
		articleFileService.deleteAttach(fileName);
		// 첨부파일 갯구 갱신
		articleFileService.updateAttachCnt(article_no);
		// 파일 삭제
		UploadFileUtils.removeFile(uploadPath, fileName);
